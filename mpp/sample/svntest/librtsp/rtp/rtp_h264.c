...
VOID * rtp_send_form_stream_func(VOID *arg)
{
    ...
    while(AbortSignal == 0)
    {
		...
		//long old_time;
		//int send_data_ret = 0;
		//static unsigned long long timepre = 0,pts = 0;
		//int nSleep = 0;
    	while(AbortSignal == 0)
    	{
			...
			if(pts > timepre && pts-timepre  < 25000)
			{
				nSleep = 25000 - (pts -timepre);		
				usleep(nSleep);
			}			
			timepre = pts;
#endif
			...
			{	
				if(rtsp[pMs->conn_index]->rtp_tcp_mode==RTP_TCP)
				{
					...
					{					
						build_rtp_tcp_nalu(pbuf->data,pbuf->size , pMs->conn_index,0,audio_encode_type,0,(pbuf->time/100.0)*TIME_STAMP_S);//(pbuf->time*TIME_STAMP_S)&0x000000003fffffffll);
					}					                      
				}
				else
				{
					if(rtsp[pMs->conn_index]->rtsp_video_stream == 1)
					{
						build_rtp_nalu(pbuf->data ,pbuf->size , pMs->conn_index,0,audio_encode_type,0,(pbuf->time/100.0)*TIME_STAMP_S);//(pbuf->time*TIME_STAMP_S)&0x000000003fffffffll);
					}				
				} 
			}  
			//usleep(10);					
