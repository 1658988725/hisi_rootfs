...
{	
    ...
    {
    	ret = create_vrtp_socket(rtsp[free_chn]->cli_rtsp.cli_host,
...
S32 rtcp_init(S32 free_chn)
{
    ...
    if(rtsp[free_chn]->rtp_tcp_mode == RTP_UDP)
    {
	    ret = create_vrtcp_socket(rtsp[free_chn]->cli_rtsp.cli_host,
...
int cdr_rtsp_create_mediasession(char *pMediaName,int nMaxFramelen,_rtsp_mediasession_cb fcb)
{
	...
	pthread_attr_setschedparam(&attr, &param);		
  	//rtsp cmd thread
  	if (pthread_create(&pMs->rtsp_send_thread, &attr, rtp_send_form_stream_func,(void*)nRet) < 0)
	{			  
