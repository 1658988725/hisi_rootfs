...
int demuxer_mp4_info(const char *in_filename,Mp4Info *pMp4InfoOut)
{
	...
	memcpy(pMp4InfoOut->oFileName, in_filename, strlen(in_filename));//这个动作最好在调用之前完成
	...
	for (i = 0; i < numTracks; i++)
	{
		...
		if (MP4_IS_VIDEO_TRACK_TYPE(trackType))
		{
			...
			pMp4InfoOut->ovFrameCount = numSamples;
		...
		else if (MP4_IS_AUDIO_TRACK_TYPE(trackType))
		{
			...
			pMp4InfoOut->oaFrameCount = MP4GetTrackNumberOfSamples(oMp4File, trackId);;
		}	
	}
	...
	sscanf(p, "%[0-9]", strtime); 
    pMp4InfoOut->tmStart  = _strtotime(strtime);  	
	//printf("pMp4InfoOut->tmStart:%lld\n", pMp4InfoOut->tmStart);
