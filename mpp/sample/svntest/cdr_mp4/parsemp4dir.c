...
void mp4dirstreamcut(void* pAgs)
{
	...
	memcpy(pMp4Info,p->oMp4Info,sizeof(Mp4Info));/*浅拷贝，*/
	...
	{	
		nReadIndex ++;
		MP4ReadSample(p->oMp4File,pMp4Info->videoindex,nReadIndex,
	...
	{	
		...
		ret = MP4ReadSample(p->oMp4File,pMp4Info->videoindex,nReadIndex,
            &pData,&nSize,&pStartTime,&pDuration,&pRenderingOffset,&pIsSyncSample);
		if(nSize<=4 || ret == false)
		{
            SAFE_FREE(pData);
			continue;
		}
	...
	{	
		...
		MP4ReadSample(p->oMp4File,pMp4Info->audioindex,nReadIndex,
            &pData,&nSize,&pStartTime,&pDuration,&pRenderingOffset,&pIsSyncSample);
    ...
    SAFE_FREE(pFrame->pData);
    SAFE_FREE(pFrame);
    SAFE_FREE(pMp4Context);
    SAFE_FREE(pMp4Info);    
}
...
*/
int cdr_read_mp4_ex(char *pTm,int len,char* dir,stream_out_cb pstreamout,void *pUserData)
{
	...
	memset(mp4ReadInfo,0x00,sizeof(Mp4ReadInfo));
 	...
 	//往前推1s 修正末尾取余数问题.
	mp4ReadInfo->tmStart  = _strtotime(pTm) - CDR_CUTMP4_EX_TIME -1;  
	...
	Mp4ReadInfo *pMp4ReadTmp = get_if(pMp4dirinfo->cutList,mp4ReadInfo->chName,mp4Readinfo_nameeq);//为查看截取列表中是否已存要截取的mp4文件
	if(pMp4ReadTmp != NULL)
	{
		 printf("%s createing....\n",mp4ReadInfo->chName);//已经存在于cutList了
         free(mp4ReadInfo);
         mp4ReadInfo = NULL;	  	 
		 return 0;
	}
	...
	if (0 == access(mp4ReadInfo->chName, W_OK))
	{
        ...
        if(oFile != NULL){//文件已存在 且非异常
           MP4Close(oFile,0);
           pstreamout(mp4ReadInfo->chName,pUserData);
           free(mp4ReadInfo);
           mp4ReadInfo = NULL;		
           return 0;
        }
	...
	if(s != NULL)
	{
		...
		mp4ReadInfo->pUserData = pUserData;
...
void *mp4dirdemuxercb(void* pAgs)
{
	...
	while ((ptr = readdir(dir)) != NULL)
	{
		...
		if(demuxer_mp4_info(chName,pMp4Item) == 0)/*将指定名字的mp4文件解析完成并将数据存于pMp4Item*/
		{
			push_if(p->fileList,pMp4Item,mp4info_nameless);/*将mp4解析后的数据挂到 p->fileList 链表中*/
            //printf("demux :%s\n",pMp4Item->oFileName);
		}else{
			SAFE_FREE(pMp4Item);
		}
...
{
	if (0 != access(fileName, F_OK)) return -1;
		
	Mp4Info *pMp4Item;
	pMp4Item = malloc(sizeof(Mp4Info));
	memset(pMp4Item,0x00,sizeof(Mp4Info));
	strncpy(pMp4Item->oFileName, fileName, strlen(fileName)+1);

	if(find_occurrence(pMp4dirinfo->fileList,pMp4Item,mp4info_nameeq))
	{
		mp4Item_free(pMp4Item);
		pMp4Item = NULL;
		return -1;
	...
	if(demuxer_mp4_info(fileName,pMp4Item) == 0)
	{
		push_if(pMp4dirinfo->fileList,pMp4Item,mp4info_nameless);
	}
	else
	{
		mp4Item_free(pMp4Item);
	...
	printf("%s add to list ok....\n",fileName);

	while(1)
	{		
		pMp4Item = get_index(pMp4dirinfo->fileList,0);
		if (0 == access(pMp4Item->oFileName, F_OK) || pMp4Item == NULL)
		{
			break;
		}
		printf("remove %s from list..\n",pMp4Item->oFileName);
		remove_front(pMp4dirinfo->fileList,mp4Item_free);
	}
...
void mp4dircutcb(void* pAgs)
{
	...
	while(p->nThreadStart)
	{
        ...
        mp4CutInfo->pStreamcb(mp4CutInfo->chName,mp4CutInfo->pUserData);//cdr_mp4cut_finish
...
int cdr_init_mp4dir(char *path)
{
	...
	ret = pthread_create(&pMp4dirinfo->tDemuxerfid, &attr,&mp4dirdemuxercb, pMp4dirinfo);
	if (ret != 0)
	{
	...
	ret = pthread_create(&pMp4dirinfo->tCutfid, NULL,(void*)mp4dircutcb, pMp4dirinfo);
	if (ret != 0)
	{
		printf("%s create thread failed..\n",__FUNCTION__);
		return -1;
