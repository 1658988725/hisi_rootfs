...
int Get_Current_DayTime(char *pTime)
{
	if (NULL == pTime)
	{
...
//pRgnContent2:第二行
int OSD_Overlay_RGN_Display_ASC16( RGN_HANDLE RgnHandle, const char *pRgnContent,
const char *pRgnContent2)
{
    HI_S32 s32Ret = HI_FAILURE;
    BITMAP_S stBitmap;
    BITMAP_S stBitmap2;
...
int OSD_Overlay_RGN_Display_ASC24( RGN_HANDLE RgnHandle, const char *pRgnContent,
const char *pRgnContent2)
{
    HI_S32 s32Ret = HI_FAILURE;
    BITMAP_S stBitmap;
...
int OSD_Overlay_RGN_Display_ASC32( RGN_HANDLE RgnHandle, const char *pRgnContent,
const char *pRgnContent2)
{
	HI_S32 s32Ret = HI_FAILURE;
	BITMAP_S stBitmap;
...
int OSD_Overlay_RGN_Display_ASC48( RGN_HANDLE RgnHandle, const char *pRgnContent )
{
	HI_S32 s32Ret = HI_FAILURE;
	BITMAP_S stBitmap;
...
char *buf_Time,unsigned int bufLen,char *ucpBuffLine2)
{
...
int get_time_part_osd(char *pucBuff,unsigned int *pBufLen)
{	
    char buf_Time[32] = {0};
    char buf_laction[32] = {0};
...
int get_EngineSpeed()
{
  ...
  iEngineSpeed = g_sCarRealtimeInformation.usEngineSpeed;
...
int get_speed()
{
  ...
  iSpeed = g_sCarRealtimeInformation.ucCarSpeed;
  
  return iSpeed;
}
unsigned char g_ucBuff[12] = "EYE KAKA ";
int get_PersonalizedSignature(char *pBuff)
{
  //10个字节
...
// 个性签名，速度， 转速
int get_osd_part_line2(char *pucBuff,unsigned int *pBufLen)
{	    
    char buf_PersSign[32] = {0};
    char buf_Speed[32] = {0};
    char buf_Engine[32] = {0};
...
int cdr_osd_sw(unsigned char ucIndex,unsigned char ucCmd)
{
    ...
    if(g_cdr_systemconfig.sCdrOsdCfg.logo == 0x00){
       ...
       if(ucOSOLogoSetFlag == 0x00){
         for (nChn = 0; nChn < RgnVencChn; nChn++){            
             RgnHandle = nChn;
		    OSD_Logo_OverlayRgn_Handle_Init(RgnHandle, nChn,32,ucCmd); 
...
int cdr_OsdInit()
{
	...
	{
		RgnHandle = nChn;
		OSD_Logo_OverlayRgn_Handle_Init( RgnHandle, nChn,32,iCmd); 
		OSD_Overlay_RGN_Display_Logo(RgnHandle,nChn); 
	}
    /*字幕*/
	RgnHandle = 0;
