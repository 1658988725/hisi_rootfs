...
static char *pucSmartUnitID = "1234567890205";//test
...
int  AppServerListen(void*pParam)
{
	int s32Socket;
    ...
    while ((s32CSocket = accept(s32Socket,(struct sockaddr*)&addrAccept,(socklen_t *)&nAddrLen)) >= 0)
    {
		...
		{                       
			if(g_appclients[i].status == APPCLIENT_IDLE)
			{
				...
				//to return ACKecho
				pthread_create(&(g_appclients[i].ClientMsgThreadID), NULL, (void*)AppClientMsg, &g_appclients[i]);
...
void cdr_app_service_init(void)
{
	...
	pthread_create(&AppServerThreadID, NULL, (void*)AppServerListen, NULL);
    pthread_create(&ClientAliveThreadID, NULL, (void*)AppClientAlive, NULL);
	pthread_create(&AckClientThreadID, NULL, (void*)AckClientPro, NULL);
...
void pack_ack_packet(int clientIndex,int cmdid,char* msgBody,int msglen)
{
	...
	memcpy(&proTmp.proheader.HeadFlag,FIX_HEADER_S,4);
...
int cdr_get_user_passwd(char *pucUserPasswd)
{
...
extern unsigned char g_usAreaMediaUploadFlag;
...
int cdr_mp4cut_finish(void* info,void *pUserData)
{
	...
	{
		char cmd[100];
		sprintf(cmd,"cp -f %s %s/%s",(char *)info,MP4CUTOUTPATH,p);
		cdr_system(cmd);		
		cdr_system("sync");
	}
...
void app_cmd_get_tmvideo(int clientIndex)
{
	...
	nRet = cdr_read_mp4_ex(strTm,len,MP4CUTOUTPATH,(stream_out_cb)&cdr_mp4cut_finish,NULL);
	if(nRet == -1)
	{
...
int ParseWifiCfg(char *pWifiCfg,int iLen)
{
  int i;
  int len[WIFI_CFG_SIZE+1] = {0};
  char *pHead = pWifiCfg;
  
  //获取每个字段占用的长度
  for(i = 1;i<WIFI_CFG_SIZE;i++)
  {    
    len[i] = get_comma(i,pWifiCfg) - get_comma(i-1,pWifiCfg) - 1;
    printf("%d ",len[i]);
  }
  //最后一个的长度
  len[WIFI_CFG_SIZE] = iLen - get_comma(WIFI_CFG_SIZE-1,pWifiCfg) ;
  printf("\n");

  if(len[1]>SIZE_OF_SSID||len[2]>SIZE_OF_SSID||len[3]>SIZE_OF_SSID||len[4]>SIZE_OF_SSID){
     printf("ssid and pwd  lens  too long\n");
     return -1;
  }
  
  pWifiCfg = pHead;
  //printf("pWifiCfg:%s \n",pWifiCfg);
  memcpy(g_sWifiCfgNumber.ucWifiMode,pWifiCfg,len[1]);//默认长度为1
  pWifiCfg=pWifiCfg+len[1]+1;
  g_sWifiCfgNumber.ucWifiMode[len[1]] = '\0';  
  //printf("g_sWifiCfgNumber.ucWifiMode:%s\n",g_sWifiCfgNumber.ucWifiMode);

  //printf("pWifiCfg2:%s\n",pWifiCfg);
  memcpy(g_sWifiCfgNumber.ucApSsid,pWifiCfg,len[2]);
  pWifiCfg=pWifiCfg+len[2]+1;
  g_sWifiCfgNumber.ucApSsid[len[2]] = '\0';
  printf("g_sWifiCfgNumber.ucApSsid %s\n",g_sWifiCfgNumber.ucApSsid);
  
  memcpy(g_sWifiCfgNumber.ucApPwd,pWifiCfg,len[3]);
  pWifiCfg=pWifiCfg+len[3]+1;
  g_sWifiCfgNumber.ucApPwd[len[3]] = '\0';
  
  memcpy(g_sWifiCfgNumber.ucStaSsid,pWifiCfg,len[4]);
  pWifiCfg=pWifiCfg+len[4]+1;
  g_sWifiCfgNumber.ucStaSsid[len[4]] = '\0';

  memcpy(g_sWifiCfgNumber.ucStaPwd,pWifiCfg,len[5]);
  pWifiCfg=pWifiCfg+len[5]+1;
  g_sWifiCfgNumber.ucStaPwd[len[5]] = '\0';


  memcpy(g_sWifiCfgNumber.ucDhcp,pWifiCfg,len[6]);
  pWifiCfg=pWifiCfg+len[6]+1;
  g_sWifiCfgNumber.ucDhcp[len[6]] = '\0';


  memcpy(g_sWifiCfgNumber.ucIpAddr,pWifiCfg,len[7]);
  pWifiCfg=pWifiCfg+len[7]+1;
  g_sWifiCfgNumber.ucIpAddr[len[7]] = '\0';
  
  memcpy(g_sWifiCfgNumber.ucGateWay,pWifiCfg,len[8]);
  pWifiCfg=pWifiCfg+len[8]+1;
  g_sWifiCfgNumber.ucGateWay[len[8]] = '\0'; 
  
  memcpy(g_sWifiCfgNumber.ucNetMask,pWifiCfg,len[9]);
  pWifiCfg=pWifiCfg+len[9]+1;
  g_sWifiCfgNumber.ucNetMask[len[9]] = '\0'; 
  
  
  memcpy(g_sWifiCfgNumber.ucDns,pWifiCfg,len[10]);
  pWifiCfg=pWifiCfg+len[10]+1;
  g_sWifiCfgNumber.ucDns[len[10]] = '\0';  
...
int CheckIp(char *pIPSrc)
{
    ...
    status=regexec(&reg,pIPSrc,nmatch,pmatch,0);  
...
int CheckIp(char *pIPSrc)
{
	...
	field=sscanf(pIPSrc,"%d . %d . %d . %d %c",&part1,&part2,&part3,&part4,&tail);  
	if(field<4|| field>5)  
	{  
		printf("expect 4 field,get %d\n",field);  
		return -1;  
	}  

	if(tail!=0)  
    {  
    	//if ipaddr like this 10.129.43.24y
	...
	{  
		printf("ip %d:%d:%d:%d/n",part1,part2,part3,part4);  
		return 0;  
	}  
	else  
	{  
...
int CheckIPCfg()
{
  if(CheckIp(g_sWifiCfgNumber.ucIpAddr) == -1)return -1;
  if(CheckIp(g_sWifiCfgNumber.ucGateWay) == -1)return -1;
  if(CheckIp(g_sWifiCfgNumber.ucNetMask) == -1)return -1;
  if(CheckIp(g_sWifiCfgNumber.ucDns) == -1)return -1;
  return 0;
}

#if 0
/*
一分钟仍无法建立连接或建立连接后被迫与热点断开时自动切换到AP模式
*/
int SetWifiAutoCfg()
{
  StartWifiTime();
...
/*
指令:"1,,,EKAKA_CDR,12345678,0,192.168.100.2,192.168.100.254,255.255.255.0,192.168.100.254"
*/
void app_cmd_set_wifi_mode(int clientIndex)
{
   //int res = 0;   
   //printf("wifi :\n%s\n",g_appclients[clientIndex].proRecvBuffer.MsgBody);//g_appclients[clientIndex].proRecvBuffer.proheader.MsgLenth
   memset(&g_sWifiCfgNumber,0,sizeof(sWifiCfg));
   ParseWifiCfg(g_appclients[clientIndex].proRecvBuffer.MsgBody,
    g_appclients[clientIndex].proRecvBuffer.proheader.MsgLenth);

#if(1)
    printf("\n****************************************\n");
    printf("ucWifiMode %s\n ucApSsid %s\n ucApPwd %s\n ucStaSsid %s\nucStaPwd %s\n ucDhcp %s\n ucIpAddr %s\n ucGateWay %s\n ucNetMask %s\n ucDns %s\n",
    g_sWifiCfgNumber.ucWifiMode,
    g_sWifiCfgNumber.ucApSsid,g_sWifiCfgNumber.ucApPwd,
    g_sWifiCfgNumber.ucStaSsid,g_sWifiCfgNumber.ucStaPwd,
    g_sWifiCfgNumber.ucDhcp,
    g_sWifiCfgNumber.ucIpAddr,g_sWifiCfgNumber.ucGateWay,
    g_sWifiCfgNumber.ucNetMask,g_sWifiCfgNumber.ucDns);
#endif

   if(!strcmp("0",g_sWifiCfgNumber.ucDhcp)){    
      printf("static ip mode \n");
      if(CheckIPCfg()==-1){
        pack_ack_packet(clientIndex,CDR_set_wifi_mode,CDR_RETURN_PARAMERROR,strlen(CDR_RETURN_PARAMERROR));   
        send_ack_data(clientIndex);
        return;
      }
   }else if(!strcmp("1",g_sWifiCfgNumber.ucDhcp)){
      printf("dhcp mode\n");
   }else{
      pack_ack_packet(clientIndex,CDR_set_wifi_mode,CDR_RETURN_PARAMERROR,strlen(CDR_RETURN_PARAMERROR));   
      send_ack_data(clientIndex);
   ...
   printf("g_sWifiCfgNumber.ucWifiMode:%02x %02x \n",g_sWifiCfgNumber.ucWifiMode[0],g_sWifiCfgNumber.ucWifiMode[1]);

   if(g_sWifiCfgNumber.ucWifiMode[0] == '1' ||
   	g_sWifiCfgNumber.ucWifiMode[0] == '2' ){
		printf("sta mode \n");
		  //sta mode
		save_cfg_to_xml("net","wifiMode",g_sWifiCfgNumber.ucWifiMode,1);
		save_cfg_to_xml("net","staSsid",g_sWifiCfgNumber.ucStaSsid,1);
		save_cfg_to_xml("net","staPasswd",g_sWifiCfgNumber.ucStaPwd,1);
		save_cfg_to_xml("net","dhcp",g_sWifiCfgNumber.ucDhcp,1);
		save_cfg_to_xml("net","ipAddr",g_sWifiCfgNumber.ucIpAddr,1);      
		save_cfg_to_xml("net","gateway",g_sWifiCfgNumber.ucGateWay,1);
		save_cfg_to_xml("net","netmask",g_sWifiCfgNumber.ucNetMask,1);
		save_cfg_to_xml("net","dns",g_sWifiCfgNumber.ucDns,1);    

		g_cdr_systemconfig.sCdrNetCfg.wifiMode = 1;
		memcpy(g_cdr_systemconfig.sCdrNetCfg.staSsid,g_sWifiCfgNumber.ucStaSsid,strlen(g_sWifiCfgNumber.ucStaSsid)+1);
		memcpy(g_cdr_systemconfig.sCdrNetCfg.staPasswd,g_sWifiCfgNumber.ucStaPwd,strlen(g_sWifiCfgNumber.ucStaPwd)+1);
		  //Update config.
		cdr_init_wifi_config();
		cdr_system("/bin/sh /app/wifi/wifi_cfg.sh sta");   
		pack_ack_packet(clientIndex,CDR_set_wifi_mode,CDR_RETURN_OK,strlen(CDR_RETURN_OK));
		send_ack_data(clientIndex);
		return;
   }
   else if(g_sWifiCfgNumber.ucWifiMode[0] == '0'){
   		printf("ap mode \n");
		//ap mode.	  
		save_cfg_to_xml("net","wifiMode",g_sWifiCfgNumber.ucWifiMode,1);
		save_cfg_to_xml("net","apSsid",g_sWifiCfgNumber.ucApSsid,1);
		save_cfg_to_xml("net","apPasswd",g_sWifiCfgNumber.ucApPwd,1);
		
		g_cdr_systemconfig.sCdrNetCfg.wifiMode = 0;
		memcpy(g_cdr_systemconfig.sCdrNetCfg.apSsid,g_sWifiCfgNumber.ucApSsid,strlen(g_sWifiCfgNumber.ucApSsid)+1);
		memcpy(g_cdr_systemconfig.sCdrNetCfg.apPasswd,g_sWifiCfgNumber.ucApPwd,strlen(g_sWifiCfgNumber.ucApPwd)+1);
		//Update config.
		cdr_init_wifi_config();
		cdr_system("/bin/sh /app/wifi/wifi_cfg.sh ap");   
		pack_ack_packet(clientIndex,CDR_set_wifi_mode,CDR_RETURN_OK,strlen(CDR_RETURN_OK));
		send_ack_data(clientIndex);
		return;
   }

	pack_ack_packet(clientIndex,CDR_set_wifi_mode,CDR_RETURN_ERROR,strlen(CDR_RETURN_ERROR));
	send_ack_data(clientIndex);
...
void send_ack_data(int clientIndex)
{
	...
	//crc 	
	unsigned short crc = get_crc16((unsigned char*)chSendData,len);	
...
{
	char crc[2] = {0};
	unsigned short s = get_crc16((unsigned char*)pData,len-2);	
	...
	switch(g_appclients[clientIndex].proRecvBuffer.proheader.CmdID)
	{
		...
		case CDR_sofeware_update 	:
         ...
         case CDR_get_gps_list:
             ...
             break;
         case CDR_set_wifi_mode://设置wifi模式
             app_cmd_set_wifi_mode(clientIndex);         
			 return;
...
int update_jpg_list_xml(void)
{
	...
	while ((ptr = readdir(dir)) != NULL){
		...
		if(strstr(ptr->d_name,"tmp") != NULL) continue;

		//去掉没有.的文件.
		if(strchr(ptr->d_name,'.') == NULL) continue;
...
{
    const char *pVideoPath = "/mnt/mmc/VIDEO";
    const char *pProtectPath = "/mnt/mmc/PROTECT";
    const char *pPhotoPath = "/mnt/mmc/PHOTO";
...
int update_sd_info_xml()
{    
    char ucTimeTemp[30] = {0};
    const char *ucFileName = "/mnt/mmc/tmp/cdr_sdinfo.xml";
...
int get_trip_time(char *pucFileName)
{
     int iTripTime = 0;
     char ucTripTime[8] = {0};//28800s = 8h 
     int i = 0;
     int len = strlen(pucFileName);
     char *ucHeader = pucFileName;
...
int cdr_syscfg_set_pro_impl(char *pRoot,char *pChildNode,char *pValue,int childflag)
{
    ...
    if(strcmp("net",pRoot)==0x00 && strcmp("apSsid",pChildNode)==0x00)
    {
      ...
      sprintf(cSedApSsidCmd,
        "sed -i '/EncrypType/ aiwpriv wlan0 set SSID=\"%s\"' /home/wifi/init_wifi_ap_7601.sh",
        pValue);
      cdr_system("sed -i '/SSID/d' /home/wifi/init_wifi_ap_7601.sh");
      cdr_system("sync");
    ...
    if(strcmp("net",pRoot)==0x00 && strcmp("apPassword",pChildNode)==0x00)
    {
       ...
       sprintf(cSedApPasswdCmd,
        "sed -i '/SSID/ aiwpriv wlan0 set WPAPSK=\"%s\"' /home/wifi/init_wifi_ap_7601.sh",
        pValue);
       cdr_system("sed -i '/WPAPSK/d' /home/wifi/init_wifi_ap_7601.sh");
       cdr_system("sync");
    ...
    {     
      ...
      save_cfg_to_xml(pRoot,pChildNode,pValue,childflag);
	  sleep(1);
	  cdr_system("reboot");
    }


    if(strcmp("net",pRoot)==0x00 && strcmp("wifiMode",pChildNode)==0x00)//wifiMode
    {

      printf("set wifi mode.............\n");
       
	  int iWifiMode = atoi(pValue);  

      save_cfg_to_xml(pRoot,pChildNode,pValue,childflag);
    
      //usleep(ECHO_DELAY_TIME);      
      if(iWifiMode == 0x00){//0x00 ap        
        SetWifiApMode();
      }
      if(iWifiMode == 0x01){//0x01 sta        
         SetWifiStaMode();
      }
    }
#endif	
