...
实时流数据推送
*/
int push_stream_callbak(int pStreamType,char* pData,unsigned int nSize,sVAFrameInfo vaFrame)
{
    Push_RtpSendFrame((unsigned char*)pData,nSize,vaFrame.u64PTS);
...
{ 
	cdr_create_live_thread();
	cdr_stream_setevent_callbak(CDR_LIVE_TYPE,&live_stream_callbak);
    cdr_stream_setevent_callbak(CDR_PHUSH_TYPE,&push_stream_callbak);
