...
extern char g_ucGpsDataBuff[300];
...
static int cdr_get_gps_data(char *pucGpsDataBuff)
{
	const char *pGpsTemp = "$GPRMC,,,,,,,,,,,,A*2C";
...
mxml_node_t * cdr_add_xml_txt_elem(mxml_node_t *parent,const char *name,const char *pValue)
{    
    mxml_node_t *item;
...
mxml_node_t * cdr_add_xml_int_elem(mxml_node_t *parent,const char *name,unsigned long pValue)
{    
    mxml_node_t *item;
...
static int cdr_get_elem_type(eRecodType eRecodType,char *eRecodTypeArr)
{   
   switch(eRecodType)
    {
    case ePHOTO:     
...
int cdr_add_log(eRecodType eRecodType, char const *pucName, char const *pucValue)
{    
	pthread_mutex_lock(&cdr_addlog_mutex);

    char ucTimeTemp[30] = {0};
    char ucTimeTempFile[10]= {0};
    char ucGprsTemp[300]    = {0};
    char ucFileName[40]    = {0};
    char eRecodTypeArr[20] = {0};
