...
static char g_cFileName[200] = {0};
static char g_cDateTime[120] = {0};
static long g_lRecCnt = 0;

extern char g_pCdrStartDateTimeBuf[120];

static int get_cdr_start_time(char *pDateTimeBuf)
{    
...
static int gp_write_header(char *pFileName,char *pDateTimeBuf)
{
   FILE *pfile = NULL;
   char pcHeadBuf[260] = {0};
...
int gp_get_file_name(char *pucGpFileName)
{
	char ucGpFileNameBuf[256] = {0};
...
{    
   ...
   fwrite(pucBuff,strlen((char *)pucBuff),1,pfile);
   fflush(pfile);
